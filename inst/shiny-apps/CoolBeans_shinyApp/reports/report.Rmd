...
```{r raw data}
raw_data_boxplots <- params$fileraw[,c(input$ncols:(input$ncols+5))] %>%
...
raw_data_boxplots <- params$fileprep[,c(input$ncols:(input$ncols+5))] %>%
  tidyr::gather(key = "metabolites", value = "value", starts_with(input$key_plot))
...
After modelling the results are the following

```{r ML}

print(params$modelresults)
