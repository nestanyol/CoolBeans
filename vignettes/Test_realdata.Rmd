...
raw_data <- raw_dataAll[, c(1, 13, 14:ncol(raw_dataAll))] # all columns
# raw_data <- raw_data%>%rename_at('combo', ~'id')
# raw_data <- raw_data%>%rename_at('BMI', ~'diet_score')
...
```{r preprocessing 1}
prep_data <- preprocess_data(raw_data, "combo", "BMI")
...
```{r train 1}
trained_model <- train_model_rf(train_data, type = 1)
trained_model
```
...
```{r results 1}
results <- test_rf_regression(trained_model, test_data)
results
```
...
```{r preprocessing 2}
prep_data <- preprocess_data(raw_data, "combo", "group")
prep_data_boxplots <- prep_data[, c(1:20)] %>%
  tidyr::gather(key = "HNC", value = "value", starts_with("HNC"))
